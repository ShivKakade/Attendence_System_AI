...
def TrackImages():
    try:
        ...
        detected_students = []  # List to store details of detected students
        detected_ids = set()    # Set to store detected student IDs
        ...
        while True:
            ...
            for (x, y, w, h) in faces:
                ...
                if conf < 50 and Id not in detected_ids:
                    ts = time.time()
                    date = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d')
                    timeStamp = datetime.datetime.fromtimestamp(ts).strftime('%H:%M:%S')
                    aa = df.loc[df['Id'] == Id]['Name'].values
                    tt = str(Id) + "-" + str(aa[0])
                    detected_students.append({'Id': Id, 'Name': str(aa[0]), 'Date': date, 'Time': timeStamp})
                    detected_ids.add(Id)   # Add ID to set of detected IDs
        ...
        if detected_students:
            attendance_df = pd.DataFrame(detected_students)
            ts = time.time()
            date = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d')
            timeStamp = datetime.datetime.fromtimestamp(ts).strftime('%H-%M-%S')
            ...
            messagebox.showinfo("Attendance", "No students detected.")
